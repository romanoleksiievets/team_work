module UserRoles
  ...
  def project_admin_permissions?(project_id, organization_id)
    organization_admin_permissions?(organization_id) ||
    (organization_member_permissions?(organization_id) &&
  ...
  def project_member_permissions?(project_id, organization_id)
    project_admin_permissions?(project_id, organization_id) ||
  ...
  def project_member?(project_id)
    ...
    ProjectsUser.where(user_id: self.id, project_id: project_id).first.present?
  end
end
